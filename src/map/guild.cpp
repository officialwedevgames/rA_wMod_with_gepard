...
int guild_recv_info(struct guild *sg) {
	...
	if((g = guild_search(sg->guild_id))==NULL) {
		...
		if ((sd = map_nick2sd(sg->master,false)) != NULL) {
			...
			// Judas Modification
			if (battle_config.guild_skill_relog_delay == 2) {
				// Nothing
			}
			else {
 				guild_block_skill(sd, battle_config.guild_skill_relog_time*60);
			}
	...
	if (g->member[0].sd && g->member[0].sd->fd) {
		...
		if (battle_config.guild_skill_relog_delay == 2) {
			// Nothing
 			...
 			guild_block_skill(g->member[0].sd, battle_config.guild_skill_relog_time*60);
		}
	}
