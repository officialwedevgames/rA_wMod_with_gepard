...
// Judas DB
void map_points_config_read(void) {
	...
	if (data != NULL) {
		...
		if ((pconfig = config_setting_get_member(settings, "points_var")) != NULL) {
			...
			CREATE(jDB->jpc, struct point_collection *, point_count);
			for (i = 0; i < point_count; i++) {
				...
				CREATE(jDB->jpc[i], struct point_collection, 1);

				jDB->jpc[i]->id = i;
				safestrncpy(jDB->jpc[i]->the_variable, aVariable, 100);
				safestrncpy(jDB->jpc[i]->the_gained_msg, aGainedMsg, 100);
				safestrncpy(jDB->jpc[i]->the_removed_msg, aRemovedMsg, 100);

			}
			jDB->points = point_count;
		}
	}
...
int do_init(int argc, char *argv[])
{
	...
	//_sleep(5000);
	jDB = &jpoints_DB;
	map_points_config_read(); // Judas Points
