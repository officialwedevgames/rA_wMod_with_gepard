...
struct map_session_data *script_rid2sdx(struct script_state *st)
{
...
* Item Destroy
*------------------------------------------*/
BUILDIN_FUNC(uniqueitem_remove4all)
{
...
BUILDIN_FUNC(rentitem) {
	...
	if (script_isstring(st, 2)) {
		...
		if (nameid <= 0 || !itemdb_exists(nameid)) {
			...
			itd = itemdb_exists(nameid);
		}
	}

	number = script_getnum(st, 3);
	minutes = script_getnum(st, 4);
	...
	//Check if it's stackable.
	if (itemdb_isstackable2(itd)) {
		uint64 myID = itemdb_unique_id(sd);

		memset(&it, 0, sizeof(it));
		it.nameid = itd->nameid;
		it.identify = 1;
		it.unique_id = myID;
		...
		if (flag == 0) {
			...
			ShowError("buildin_rentitem: User is overweight.\n");
			return SCRIPT_CMD_FAILURE;
		}
		else {
			ShowError("buildin_rentitem: Error occurred.\n");
			return SCRIPT_CMD_FAILURE;
		}
	...
	if (itd->type == IT_PETEGG || itd->type == IT_PETARMOR) {
		ShowError("buildin_rentitem: Cannot create rented pet eggs or pet armors.\n");
		return SCRIPT_CMD_FAILURE;
	}

	// if not pet egg
	if (!pet_create_egg(sd, itd->nameid)) {
...
BUILDIN_FUNC(successrefitem) {
	...
	if (i >= 0) {
		...
		struct item_data *ditem = sd->inventory_data[i];
		// Judas Refine Message
		if (ditem->wlv == 0 && sd->inventory.u.items_inventory[i].refine >= battle_config.announce_refine_armor || ditem->wlv == 3 && sd->inventory.u.items_inventory[i].refine >= battle_config.announce_refine_weapon_lvl_3 || ditem->wlv == 4 && sd->inventory.u.items_inventory[i].refine >= battle_config.announce_refine_weapon_lvl_4) {
			char message[300];
			struct item_data* item_data;
			item_data = itemdb_search(sd->inventory_data[i]->nameid);
			sprintf(message, "Citizens of Midgard, Lady Luck shines upon [%s]!! Congratulations for refining [%s] to [%d].", sd->status.name, item_data->jname, sd->inventory.u.items_inventory[i].refine);
			intif_broadcast(message, (int)strlen(message) + 1, BC_DEFAULT);
		}
		else if (ditem->wlv == 1 && sd->inventory.u.items_inventory[i].refine >= battle_config.announce_refine_weapon_lvl_1 || ditem->wlv == 2 && sd->inventory.u.items_inventory[i].refine >= battle_config.announce_refine_weapon_lvl_1) {
			char message[300];
			struct item_data* item_data;
			item_data = itemdb_search(sd->inventory_data[i]->nameid);
			sprintf(message, "Citizens of Midgard, Lady Luck shines upon [%s]!! Congratulations for refining [%s] to [%d].", sd->status.name, item_data->jname, sd->inventory.u.items_inventory[i].refine);
...
BUILDIN_FUNC(tpoint)
{
	struct map_session_data *sd = script_rid2sdx(st);
	...
	if (result < 0) {
		...
		if (value > 0) {
			char the_msg[150];
			sprintf(the_msg, "Gained %d Tier Points. Total %d Points", value, result);
			clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], the_msg, false, SELF);
		}
		else {
			char the_msg[150];
			sprintf(the_msg, "Removed %d Tier Points. Total %d Points", -value, result);
			clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], the_msg, false, SELF);
		}
	}
...
BUILDIN_FUNC(checktierpoint)
{
	int result = 0;
	int account = script_getnum(st, 2);

	struct map_session_data *sd = script_rid2sdx(st);
	struct map_session_data *qsd = map_id2sd(account);

	if (qsd == NULL) {
		result = clif_query_tier_points(sd);
	}
	else {
		if (account < 200000) {
			clif_displaymessage(sd->fd, "Unkown Account");
			return false;
		}
		result = clif_query_tier_points(qsd);
	}
...
/**
* Judas Reload Roulette
**/
BUILDIN_FUNC(reloadroulette)
{
	struct s_mapiterator* iter;
...
BUILDIN_FUNC(resetroulette)
{
	char roulette_db[256] = "db_roulette";
	if (SQL_ERROR == Sql_Query(mmysql_handle, "TRUNCATE TABLE `%s`", roulette_db))
...
BUILDIN_FUNC(setnpcdisplay)
{
	...
	if( class_ != JT_FAKE_NPC && nd->class_ != class_ )
...
BUILDIN_FUNC(points)
{
	...
	if (is_string_variable(name))
		set_reg(st, sd, num, name, script_getstr(st, 3), script_getref(st, 2));
	else
		set_reg(st, sd, num, name, (const void *)__64BPRTSIZE(script_getnum(st, 3)), script_getref(st, 2));
...
BUILDIN_FUNC(openshop) 
{
	...
	if( script_hasdata(st, 2) ) {
		name = script_getstr(st, 2);
		if( !(nd = npc_name2id(name)) || nd->subtype != NPCTYPE_SCRIPT ) {
...
BUILDIN_FUNC(setcurrency) 
{
	...
	nd->trader_funds[0] = val1;
	nd->trader_funds[1] = val2;
	
	return SCRIPT_CMD_SUCCESS;
}

// Have to pass in # or without #
// points_account_db: #ROKPOINTS, #LOYALPOINTS, #VOTEPOINTS, #REFERRALPOINTS
// points_char_db : ROKPOINTS, LOYALPOINTS, VOTEPOINTS, REFERRALPOINTS
BUILDIN_FUNC(setnewcurrency)
{
	const char* pointName;
	pointName = script_getstr(st, 2);
	...
	if (sd == NULL) {
		ShowWarning("buildin_setcurrency: Could not attach player!\n");
		return false;
	}
	// points_account_db - W/ Hash
	// points_char_db    - W/o Hash
	val1 = pc_readaccountreg(sd, add_str(pointName));
...
BUILDIN_FUNC(tradertype) 
{
	...
	if (strcmp(str, "NST_CASH") == 0) {
		type = NST_CASH;
	}
	if (strcmp(str, "NST_CUSTOM") == 0) {
		type = NST_CUSTOM;
	}
	...
	if( !(nd = map_id2nd(st->oid)) ) {
		...
		return false;
	} else if ( type < 0 || type > 4 ) { // Change 4 if not the MAX
...
struct script_function buildin_func[] = {
	...
	BUILDIN_DEF(tradertype, "s"),
	BUILDIN_DEF(setnewcurrency, "s"), // Judas Currency
	...
	BUILDIN_DEF(points, "rv?"),
	BUILDIN_DEF(tpoint, "i"),		   // Judas Tier
	BUILDIN_DEF(resettier, ""),		   // Judas Tier
	BUILDIN_DEF(checktierpoint, "?"),  // Judas Tier
	...
	// Judas Unique
	BUILDIN_DEF(uniqueitem_remove4all, "i"),
	...
	BUILDIN_DEF(reloadroulette, "?"), // Judas Reload Roulette
	BUILDIN_DEF(resetroulette, "?"),  // Judas Reset Roulette
