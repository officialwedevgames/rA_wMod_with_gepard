...
int status_change_spread(struct block_list *src, struct block_list *bl, bool type);
	...
	unsigned short status_base_matk(struct block_list *bl, const struct status_data* status, int level);unsigned short status_base_matk_max(struct block_list *bl, const struct status_data* status, int level);
#endif
