...
void clif_buylist(struct map_session_data *sd, struct npc_data *nd)
{
	...
	for (i = 0; i < shop_size; i++) {
		if (shop[i].nameid) {
			...
			// Judasu Shop
			if ((nd->u.scr.flag == 2 && nd->subtype == NPCTYPE_SHOP || nd->u.scr.flag == 2 && nd->subtype == NPCTYPE_SCRIPT) || (nd->u.scr.flag == 3 && nd->subtype == NPCTYPE_SHOP || nd->u.scr.flag == 3 && nd->subtype == NPCTYPE_SCRIPT)) { // Don't display Discount value [Cydh]
...
void clif_selllist(struct map_session_data *sd, struct npc_data *nd)
{
	...
	WFIFOW(fd,0)=0xc7;
	// Judasu Shop
	if ((nd->u.scr.flag == 1 && nd->subtype == NPCTYPE_SHOP || nd->u.scr.flag == 1 && nd->subtype == NPCTYPE_SCRIPT) || (nd->u.scr.flag == 3 && nd->subtype == NPCTYPE_SHOP || nd->u.scr.flag == 3 && nd->subtype == NPCTYPE_SCRIPT)) { // Don't display Overcharged value [Cydh]
		overcharge = false;
	}
	for( i = 0; i < MAX_INVENTORY; i++ )
	{
		if( sd->inventory.u.items_inventory[i].nameid > 0 && sd->inventory_data[i] )
		{
			...
			WFIFOL(fd, 10 + c * 10) = (overcharge) ? pc_modifysellvalue(sd, val) : val; // Judasu Shop
			c++;
		}
	}
	WFIFOW(fd,2)=c*10+4;
...
void clif_cashshop_open( struct map_session_data* sd ){
	...
	// Judas Points
	if (battle_config.cashshop_db == 1) { // 1 = acc_reg_num_db {
		WFIFOL(sd->fd, 2) = sd->cashPoints; //[Ryuuzaki] - switched positions to reflect proper values
	}
	else if (battle_config.cashshop_db == 2) { // 2 = points_account_db
		WFIFOL(sd->fd, 2) = pc_readaccountreg(sd, add_str("#ROKPOINTS"));
	}
...
void clif_cashshop_show(struct map_session_data *sd, struct npc_data *nd)
{
	...
	if( nd->subtype == NPCTYPE_SCRIPT ) {
		...
		currency[0] = nd->trader_funds[0];
		currency[1] = nd->trader_funds[1];
	} else {
		shop = nd->u.shop.shop_item;
		shop_size = nd->u.shop.count;
		...
		// Judas Points
		if (battle_config.cashshop_db == 1) { // 1 = acc_reg_num_db {
			npc_shop_currency_type(sd, nd, currency, true);
		}
		else if (battle_config.cashshop_db == 2) { // 2 = points_account_db
			currency[0] = pc_readaccountreg(sd, add_str("#ROKPOINTS"));
	...
	for( i = 0; i < shop_size; i++ ) {
		if( shop[i].nameid ) {
			...
			// Vip Discount
			if (pc_isvip(sd) && nd->u.scr.shop->type == NST_CASH) {
				WFIFOL(fd,offset+4+i*11) = (unsigned int)cap_value(shop[i].value * (1 - (battle_config.vip_cash_discount / 100.)), 0, UINT_MAX); // Discount Price
			}
			else {	
...
// Judas Tier
void clif_truncate_tier_points() {
	if (SQL_ERROR == Sql_Query(mmysql_handle, "TRUNCATE TABLE `tier_db`")) {
		Sql_ShowDebug(mmysql_handle);
	}
}

// Judas Tier
int clif_handle_tier_points(struct map_session_data *sd, int points) {
	char* data;
	int acctPoints = 0;
	int totalPoints = 0;

	if (SQL_ERROR == Sql_Query(mmysql_handle, "SELECT points FROM tier_db WHERE account_id = '%d'", sd->status.account_id)) {
		Sql_ShowDebug(mmysql_handle);
	}

	if (Sql_NumRows(mmysql_handle) > 0) {
		while (SQL_SUCCESS == Sql_NextRow(mmysql_handle)) {
			Sql_GetData(mmysql_handle, 0, &data, NULL); acctPoints = atoi(data);
		}
		Sql_FreeResult(mmysql_handle);
		totalPoints = points + acctPoints;

		if (acctPoints < 0) {
			return -1; // Can't reduce any further
		}

		else if (totalPoints < 0) { // Set to 0
			if (SQL_ERROR == Sql_Query(mmysql_handle, "UPDATE tier_db set points = 0 where account_id = '%d'",
				sd->status.account_id
			)) {
				Sql_ShowDebug(mmysql_handle);
			}
			...
			if (points > 0) {
				if (SQL_ERROR == Sql_Query(mmysql_handle, "UPDATE tier_db set points = points + '%d' where account_id = '%d'",
					points,
					sd->status.account_id
				)) {
					Sql_ShowDebug(mmysql_handle);
				...
				if (SQL_ERROR == Sql_Query(mmysql_handle, "UPDATE tier_db set points = '%d' where account_id = '%d'",
					totalPoints,
					sd->status.account_id
				)) {
					Sql_ShowDebug(mmysql_handle);
		...
		if (SQL_ERROR == Sql_Query(mmysql_handle, "INSERT INTO `%s` (`account_id`,`points`) VALUES ('%d','%d')",
			"tier_db",
			sd->status.account_id,
			points
		)) {
...
// Judas Tier
int clif_query_tier_points(struct map_session_data *sd) {
	char* data;
	int acctPoints = 0;
	...
	if (Sql_NumRows(mmysql_handle) > 0) {
		...
		if (acctPoints < 0) {
			return 0; // Can't reduce any further
		}
	}
...
void clif_cashshop_ack(struct map_session_data* sd, int error)
{
	...
	if( (nd = map_id2nd(sd->npc_shopid)) && nd->subtype == NPCTYPE_SCRIPT ) {
		npc_trader_count_funds(nd,sd);
		currency[0] = nd->trader_funds[0];
		currency[1] = nd->trader_funds[1];
	} else {
		// Judas Points
		if (battle_config.cashshop_db == 1) { // 1 = acc_reg_num_db {
			npc_shop_currency_type(sd, nd, currency, false);
		}
		else if (battle_config.cashshop_db == 2) { // 2 = points_account_db
			currency[0] = pc_readaccountreg(sd, add_str("#ROKPOINTS"));
			currency[1] = sd->kafraPoints;
...
void clif_cashshop_result( struct map_session_data *sd, unsigned short item_id, uint16 result ){
	...
	// Judas Points
	if (battle_config.cashshop_db == 1) { // 1 = acc_reg_num_db {
		WFIFOL(sd->fd, 8) = sd->cashPoints;/* current cash point */
	}
	else if (battle_config.cashshop_db == 2) { // 2 = points_account_db
		WFIFOL(sd->fd, 8) = pc_readaccountreg(sd, add_str("#ROKPOINTS"));
	}
