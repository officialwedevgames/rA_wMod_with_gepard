...
#include "pc_groups.hpp" // script_config
	...
	switch( nd->u.scr.shop->type ) {
		...
		case NST_MARKET: {
			...
			if (i == nd->u.scr.shop->items) {
				clif_messagecolor(&sd->bl, color_table[COLOR_RED], msg_txt(sd,881), false, SELF);
				return false;
			}
...
void npc_trader_count_funds(struct npc_data *nd, struct map_session_data *sd) {
	...
	nd->trader_funds[0] = nd->trader_funds[1] = 0;/* clear */

	switch (nd->u.scr.shop->type) {
	case NST_CASH:
		// Judas Points
		if (battle_config.cashshop_db == 1) { // 1 = acc_reg_num_db
			nd->trader_funds[0] = sd->cashPoints;
		}
		else if (battle_config.cashshop_db == 2) { // 2 = points_account_db
			nd->trader_funds[0] = pc_readaccountreg(sd, add_str("#ROKPOINTS"));
		}
		else {
			ShowError("npc_trader_count_funds: unsupported shop type %d\n", nd->u.scr.shop->type);
			return;
		}
		nd->trader_funds[1] = sd->kafraPoints;
		return;
	case NST_CUSTOM:
		break;
	default:
...
int npc_buysellsel(struct map_session_data* sd, int id, int type)
{
	...
	if (nd->subtype != NPCTYPE_SHOP && !(nd->subtype == NPCTYPE_SCRIPT && nd->u.scr.shop && nd->u.scr.shop->items)) {
...
static enum e_CASHSHOP_ACK npc_cashshop_process_payment(struct npc_data *nd, int price, int points, struct map_session_data *sd) {
	...
	switch(nd->subtype) {
		case NPCTYPE_CASHSHOP:
			if (battle_config.cashshop_db == 2) { // 2 = points_account_db
				if (sd->kafraPoints < points || pc_readaccountreg(sd, add_str("#ROKPOINTS")) < (price - points))
			...
			// Add Tier Points
			clif_handle_tier_points(sd, price);
			pc_paycash(sd, price, points, LOG_TYPE_NPC, "#ROKPOINTS");
			break;
		case NPCTYPE_ITEMSHOP:
			{
...
int npc_cashshop_buylist(struct map_session_data *sd, int points, int count, unsigned short* item_list)
{
	...
	for( i = 0; i < count; i++ )
	{
		...
		// Vip Discount
		if ( pc_isvip(sd) && nd->u.scr.shop->type == NST_CASH && pc_has_permission(sd, PC_PERM_VIP_CASH_DISCOUNT) || sd->group_id == battle_config.partner_account && nd->u.scr.shop->type == NST_CASH ) {
			vt += ((unsigned int)cap_value(shop[j].value * (1 - (battle_config.vip_cash_discount / 100.)), 0, UINT_MAX)) * amount;
		}
		else {
	...
	if( nd->subtype == NPCTYPE_SCRIPT && nd->u.scr.shop->type == NST_CUSTOM ) {
		if( !npc_trader_pay(nd,sd,vt,points) )
			return 6;
	}
	else {
	...
	for( i = 0; i < count; i++ ) {
		...
		if( !pet_create_egg(sd,nameid) ) {
			...
			// Judas Bound/Rental -- Find correct index
			ARR_FIND(0, shop_size, j, shop[j].nameid == nameid);
			if (j == shop_size || shop[j].value <= 0)
				return ERROR_TYPE_ITEM_ID;
			...
			// Judas Bound/Rental
			if (shop[j].rental > 0) {
				item_tmp.expire_time = (unsigned int)(time(NULL) + (shop[j].rental * 60));
			}
			else {
				if (shop[j].bound > 0) {
					item_tmp.bound = shop[j].bound;
				}
			}
...
int npc_cashshop_buy(struct map_session_data *sd, unsigned short nameid, int amount, int points)
{
	...
	if ((double)shop[i].value * amount > INT_MAX)
	{
		...
		ShowDebug("(NPC:'%s' (%s,%d,%d), player:'%s' (%d/%d), value:%d, amount:%d)\n",
					nd->exname, map_mapid2mapname(nd->bl.m), nd->bl.x, nd->bl.y, sd->status.name, sd->status.account_id, sd->status.char_id, shop[i].value, amount);
		return ERROR_TYPE_ITEM_ID;
	}
...
uint8 npc_buylist(struct map_session_data* sd, uint16 n, struct s_npc_buy_list *item_list) {
	...
	if( nd->subtype != NPCTYPE_SHOP ) {
		if( nd->subtype == NPCTYPE_SCRIPT && nd->u.scr.shop && nd->u.scr.shop->type == NST_ZENY ) {
			shop = nd->u.scr.shop->item;
			shop_size = nd->u.scr.shop->items;
		} else
			return 3;
	...
	for( i = 0; i < n; ++i ) {
		...
		// No Discount [Cydh] ~ Judasu Shop
		if (nd->subtype == NPCTYPE_SHOP || nd->subtype == NPCTYPE_SCRIPT) {
			if (nd->u.scr.flag == 1) {
				value = pc_modifybuyvalue(sd, value);
			}
			else if (nd->u.scr.flag == 0) {
				value = pc_modifybuyvalue(sd, value);
			}
		}
...
static const char* npc_parse_shop(char* w1, char* w2, char* w3, char* w4, const char* start, const char* buffer, const char* filepath)
{
	...
	npc_parsename(nd, w3, start, buffer, filepath);
	nd->class_ = m == -1 ? JT_FAKE_NPC : npc_parseview(w4, start, buffer, filepath);
	nd->speed = 200;
...
static const char* npc_parse_script(char* w1, char* w2, char* w3, char* w4, const char* start, const char* buffer, const char* filepath, int options) {
	...
	if (options&NPO_TRADER) {
		...
		// Judasu Shop
		int NotNumber = 0;
		while (*w2) { // While there are more characters to process...
			if (isdigit(*w2)) { // Upon finding a digit, ...
				nd->u.scr.flag = atoi(w2);
				NotNumber = 0;
				break;
			}
...
const char* npc_parse_duplicate(char* w1, char* w2, char* w3, char* w4, const char* start, const char* buffer, const char* filepath, int options)
{
	...
	if( type == NPCTYPE_SCRIPT && options&NPO_ONINIT ) {
...
int npc_duplicate4instance(struct npc_data *snd, int16 m) {
	...
	if( snd->subtype == NPCTYPE_WARP ) { // Adjust destination, if instanced
		...
		npc_parse_duplicate(w1, w2, w3, w4, stat_buf, stat_buf, "INSTANCING", NPO_NONE);
	}
...
int npc_parsesrcfile(const char* filepath, bool runOnInit)
{
	...
	for( p = skip_space(buffer); p && *p ; p = skip_space(p) )
	{
		...
		else if (strcmp(w2, "trader") == 0 || strcmp(w2, "trader1") == 0 || strcmp(w2, "trader2") == 0 || strcmp(w2, "trader3") == 0) { // Judasu Shop
			p = npc_parse_script(w1, w2, w3, w4, p, buffer, filepath, (runOnInit ? NPO_ONINIT : NPO_NONE) | NPO_TRADER);
		}
		else if( (i=0, sscanf(w2,"duplicate%n",&i), (i > 0 && w2[i] == '(')) && count > 3 )
			p = npc_parse_duplicate(w1,w2,w3,w4, p, buffer, filepath, (runOnInit?NPO_ONINIT:NPO_NONE));
