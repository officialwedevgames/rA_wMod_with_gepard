...
static char variable[NAME_LENGTH];
static char atcmd_player_name[NAME_LENGTH];
const char *parent_cmd;

// Judas Warp
static char NpcList[100][NAME_LENGTH];
...
ACMD_FUNC(go)
{
	...
	const struct {
		...
		{ MAP_PRONTERA,    116,  73 }, //  0=Prontera
		{ MAP_MORROC,      156,  46 }, //  1=Morroc
		{ MAP_GEFFEN,      119,  40 }, //  2=Geffen
		{ MAP_PAYON,        70, 100 }, //  3=Payon
		{ MAP_ALBERTA,     117,  57 }, //  4=Alberta
		{ MAP_IZLUDE,       94, 103 }, //  5=Izlude
		{ MAP_ALDEBARAN,   168, 112 }, //  6=Al de Baran
		{ MAP_LUTIE,       147, 134 }, //  7=Lutie
		{ MAP_COMODO,      207, 144 }, //  8=Comodo
...
{
	char item_name[100];
	int number = 0, item_id, flag = 0, minutes = 0;
	struct item item_tmp;
	struct item_data *item_data;
	...
	if (!message || !*message || (
		sscanf(message, "\"%99[^\"]\" %d %d", item_name, &number, &minutes) < 1 &&
		sscanf(message, "%99s %d %d", item_name, &number, &minutes) < 1))
	{
		clif_displaymessage(fd, "Please enter an item name or ID (usage: @rentitem <item name/ID> <amount> <minutes>).");
		return 0;
	}
	...
	//Check if it's stackable.
	if (itemdb_isstackable2(item_data)) {
		uint64 myID = itemdb_unique_id(sd);

		memset(&item_tmp, 0, sizeof(item_tmp));
		item_tmp.nameid = item_id;
		item_tmp.identify = 1;
		item_tmp.unique_id = myID;
		item_tmp.expire_time = (unsigned int)(time(NULL) + (minutes * 60));

		// Generated our unique item
		flag = pc_additem(sd, &item_tmp, number, LOG_TYPE_COMMAND);

		if (flag == 0) {
		}
		else if (flag == 2) {
			clif_displaymessage(fd, "You are overweight.");
			return -1;
		}
		else {
			clif_displaymessage(fd, "Error occurred.");
			return -1;
		}
	...
	if (item_data->type == IT_PETEGG || item_data->type == IT_PETARMOR) {
		clif_displaymessage(fd, "Cannot create rented pet eggs or pet armors.");
		return -1;
	}
...
ACMD_FUNC(rentitem2)
{
	...
	char item_name[100];
	int number = 0, minutes = 0;
	struct item_data *item_data;

	memset(item_name, '\0', sizeof(item_name));

	if (!message || !*message || (
		sscanf(message, "\"%99[^\"]\" %d %d %d %d %d %d %d", item_name, &number, &minutes, &ref, &c1, &c2, &c3, &c4) < 8 &&
		sscanf(message, "%99s %d %d %d %d %d %d %d", item_name, &number, &minutes, &ref, &c1, &c2, &c3, &c4) < 8))
	{
		clif_displaymessage(fd, "Please enter an item name or ID (usage: @rentitem2 <itemid>,<amount>,<minutes>,<refine>,<card1>,<card2>,<card3>,<card4>).");
		return 0;
	}
	...
	if (ref > MAX_REFINE) {
		clif_displaymessage(fd, "Invalid Refinement...");
		return -1;
	}
	else {
	...
	if (itemdb_isstackable2(item_data)) {
		...
		it.card[2] = (short)c3;
		it.card[3] = (short)c4;
		it.expire_time = (unsigned int)(time(NULL) + (minutes * 60));

		// Generated our unique item
		flag = pc_additem(sd, &it, number, LOG_TYPE_COMMAND);
...
ACMD_FUNC(reloadroulette)
{
	...
	iter = mapit_getallusers();
	for (pl_sd = (TBL_PC*)mapit_first(iter); mapit_exists(iter); pl_sd = (TBL_PC*)mapit_next(iter)) {
		pl_sd->state.roulette_reloaded = 1;
	}

	mapit_free(iter);
	itemdb_parse_roulette_db();
	safestrncpy(tmp_soutput, "Roulette has been reloaded and inactive until you re-login. The new item list will only appear when you reopen the client.", sizeof(tmp_soutput));
...
ACMD_FUNC(addwarp)
{
	...
	for (int c = 0; c < 100; c++)
	{
		if (strlen(NpcList[c]) == 0)
		{
			strncpy(NpcList[c], nd->exname, strlen(nd->exname));
			break;
		}
	}
...
// Judas Warp + Nasagnilac
ACMD_FUNC(removewarpall)
{
	struct npc_data *nd;
	...
	for (int c = 0; c < 100; c++)
	{
		...
		if ((nd = npc_name2id(NpcList[c])) == NULL)
		{
			clif_displaymessage(sd->fd, "All added warps are now removed.");
			break;
		}else{
			// It does exist
			npc_unload_duplicates(nd);
...
ACMD_FUNC(removewarp)
{
	...
	if (!*message || sscanf(message, "%24[^\n]", NPCname) < 1) {
		clif_displaymessage(sd->fd, "Usage: @removewarp <NPC_name>");
		return -1;
	}
...
#endif
				if (pc_isvip(sd) || sd->group_id == battle_config.partner_account) // Display item rate increase for VIP + Modified Partner Account
...
ACMD_FUNC(rates)
{
	...
	// Modified Partner Account
	snprintf(buf, CHAT_SIZE_MAX, msg_txt(sd, 1298), // Experience rates: Base %.2fx / Job %.2fx
		(battle_config.base_exp_rate + (pc_isvip(sd) || sd->group_id == battle_config.partner_account ? (battle_config.vip_base_exp_increase * battle_config.base_exp_rate) / 100 : 0)) / 100.,
		(battle_config.job_exp_rate + (pc_isvip(sd) || sd->group_id == battle_config.partner_account ? (battle_config.vip_job_exp_increase * battle_config.job_exp_rate) / 100 : 0)) / 100.);
	...
	snprintf(buf, CHAT_SIZE_MAX, msg_txt(sd, 1299), // Normal Drop Rates: Common %.2fx / Healing %.2fx / Usable %.2fx / Equipment %.2fx / Card %.2fx
		(battle_config.item_rate_common + (pc_isvip(sd) || sd->group_id == battle_config.partner_account ? (battle_config.vip_drop_increase * battle_config.item_rate_common) / 100 : 0)) / 100.,
		(battle_config.item_rate_heal + (pc_isvip(sd) || sd->group_id == battle_config.partner_account ? (battle_config.vip_drop_increase * battle_config.item_rate_heal) / 100 : 0)) / 100.,
		(battle_config.item_rate_use + (pc_isvip(sd) || sd->group_id == battle_config.partner_account ? (battle_config.vip_drop_increase * battle_config.item_rate_use) / 100 : 0)) / 100.,
		(battle_config.item_rate_equip + (pc_isvip(sd) || sd->group_id == battle_config.partner_account ? (battle_config.vip_drop_increase * battle_config.item_rate_equip) / 100 : 0)) / 100.,
		(battle_config.item_rate_card + (pc_isvip(sd) || sd->group_id == battle_config.partner_account ? (battle_config.vip_drop_increase * battle_config.item_rate_card) / 100 : 0)) / 100.);
	clif_displaymessage(fd, buf);
	snprintf(buf, CHAT_SIZE_MAX, msg_txt(sd, 1300), // Boss Drop Rates: Common %.2fx / Healing %.2fx / Usable %.2fx / Equipment %.2fx / Card %.2fx
		(battle_config.item_rate_common_boss + (pc_isvip(sd) || sd->group_id == battle_config.partner_account ? (battle_config.vip_drop_increase * battle_config.item_rate_common_boss) / 100 : 0)) / 100.,
		(battle_config.item_rate_heal_boss + (pc_isvip(sd) || sd->group_id == battle_config.partner_account ? (battle_config.vip_drop_increase * battle_config.item_rate_heal_boss) / 100 : 0)) / 100.,
		(battle_config.item_rate_use_boss + (pc_isvip(sd) || sd->group_id == battle_config.partner_account ? (battle_config.vip_drop_increase * battle_config.item_rate_use_boss) / 100 : 0)) / 100.,
		(battle_config.item_rate_equip_boss + (pc_isvip(sd) || sd->group_id == battle_config.partner_account ? (battle_config.vip_drop_increase * battle_config.item_rate_equip_boss) / 100 : 0)) / 100.,
		(battle_config.item_rate_card_boss + (pc_isvip(sd) || sd->group_id == battle_config.partner_account ? (battle_config.vip_drop_increase * battle_config.item_rate_card_boss) / 100 : 0)) / 100.);
	clif_displaymessage(fd, buf);
	snprintf(buf, CHAT_SIZE_MAX, msg_txt(sd, 1024), // MVP Drop Rates: Common %.2fx / Healing %.2fx / Usable %.2fx / Equipment %.2fx / Card %.2fx
		(battle_config.item_rate_common_mvp + (pc_isvip(sd) || sd->group_id == battle_config.partner_account ? (battle_config.vip_drop_increase * battle_config.item_rate_common_mvp) / 100 : 0)) / 100.,
		(battle_config.item_rate_heal_mvp + (pc_isvip(sd) || sd->group_id == battle_config.partner_account ? (battle_config.vip_drop_increase * battle_config.item_rate_heal_mvp) / 100 : 0)) / 100.,
		(battle_config.item_rate_use_mvp + (pc_isvip(sd) || sd->group_id == battle_config.partner_account ? (battle_config.vip_drop_increase * battle_config.item_rate_use_mvp) / 100 : 0)) / 100.,
		(battle_config.item_rate_equip_mvp + (pc_isvip(sd) || sd->group_id == battle_config.partner_account ? (battle_config.vip_drop_increase * battle_config.item_rate_equip_mvp) / 100 : 0)) / 100.,
		(battle_config.item_rate_card_mvp + (pc_isvip(sd) || sd->group_id == battle_config.partner_account ? (battle_config.vip_drop_increase * battle_config.item_rate_card_mvp) / 100 : 0)) / 100.);
	clif_displaymessage(fd, buf);
	snprintf(buf, CHAT_SIZE_MAX, msg_txt(sd, 1301), // Other Drop Rates: MvP %.2fx / Card-Based %.2fx / Treasure %.2fx
		(battle_config.item_rate_mvp + (pc_isvip(sd) || sd->group_id == battle_config.partner_account ? (battle_config.vip_drop_increase * battle_config.item_rate_mvp) / 100 : 0)) / 100.,
		(battle_config.item_rate_adddrop + (pc_isvip(sd) || sd->group_id == battle_config.partner_account ? (battle_config.vip_drop_increase * battle_config.item_rate_adddrop) / 100 : 0)) / 100.,
		(battle_config.item_rate_treasure + (pc_isvip(sd) || sd->group_id == battle_config.partner_account ? (battle_config.vip_drop_increase * battle_config.item_rate_treasure) / 100 : 0)) / 100.);
	clif_displaymessage(fd, buf);
...
ACMD_FUNC(tpoint) {
	...
	int result = clif_handle_tier_points(sd, value);

	if (result < 0) {
		clif_displaymessage(sd->fd, "Unable to decrease the number/value. Tier Points set to 0");
	}
	else {
		if (value > 0) {
			sprintf(atcmd_output, "Gained %d Tier Points. Total %d Points", value, result);
			clif_displaymessage(fd, atcmd_output);
		}
		else {
			sprintf(atcmd_output, "Removed %d Tier Points. Total %d Points", -value, result);
			clif_displaymessage(fd, atcmd_output);
		}
	}
...
ACMD_FUNC(resettier) {
	clif_truncate_tier_points();
	clif_displaymessage(sd->fd, "All tier points has been reset.");
	return true;
}
...
ACMD_FUNC(cash)
{
	...
	if( !strcmpi(parent_cmd,"cash") )
	{
		if( value > 0 ) {
			...
			if (-value > sd->cashPoints) //By command, if cash < value, force it to remove all
				value = -sd->cashPoints;
			if( (ret=pc_paycash(sd, -value, 0, LOG_TYPE_COMMAND, NULL)) >= 0){
				// If this option is set, the message is already sent by pc function
				if( !battle_config.cashshop_show_points ){
	...
	{ // @points
		if( value > 0 ) {
			...
			if( (ret=pc_paycash(sd, -value, -value, LOG_TYPE_COMMAND, NULL)) >= 0){
...
ACMD_FUNC(points)
{
	char output[128];
	int amount = 0;
	int ret = 0; //char output[128];

	memset(atcmd_output, '\0', sizeof(atcmd_output));
	memset(variable, '\0', sizeof variable);

	if (!*message || sscanf(message, "%255s %12d", &variable, &amount) < 2) {
		clif_displaymessage(fd, "The correct usage is: @points <variable> <amount>");
		return false;
	}
	else if (amount == 0) {
		clif_displaymessage(fd, msg_txt(sd, 1322)); // Please enter an amount.
		...
		for (int x = 0; x < jDB->points; x++) {
			if (strcmp(variable, jDB->jpc[x]->the_variable) == 0) {
				foundIt = 1;
				theIndex = x;
				break;
			}
		...
		if (foundIt == 0) {
			clif_displaymessage(fd, "Specific points could not been found");
			return false;
		}

		if ((ret = pc_paycash(sd, -amount, 0, LOG_TYPE_COMMAND, jDB->jpc[theIndex]->the_variable)) >= 0) {
			if (battle_config.cashshop_show_points) {
				sprintf(output, jDB->jpc[theIndex]->the_removed_msg, ret, pc_readaccountreg(sd, add_str(jDB->jpc[theIndex]->the_variable)));
			}
			clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], output, false, SELF);
		}
		else
		...
		if ((ret = pc_getpoints(sd, variable, amount)) >= 0) {
			// If this option is set, the message is already sent by pc function
			if (battle_config.cashshop_show_points) {
				if (!strcmpi(variable, jDB->jpc[theIndex]->the_variable)) {
					sprintf(output, jDB->jpc[theIndex]->the_gained_msg, ret, pc_readaccountreg(sd, add_str(jDB->jpc[theIndex]->the_variable)));
				}
				clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], output, false, SELF);
			}
		}
		else
			clif_displaymessage(fd, msg_txt(sd, 149)); // Unable to decrease the number/value.	
	}
	return 0;
}
...
#include "../custom/atcommand_def.inc"
		ACMD_DEF(itemdestroy),		// Judas Unique
		ACMD_DEF(uniqueitemdestroy),
		...
		ACMD_DEF(snow),
		ACMD_DEF(reloadroulette), // Judas Reload Roulette
		ACMD_DEF(sakura),
		ACMD_DEF(clouds),
		ACMD_DEF(clouds2),
		ACMD_DEF(fog),
		...
		ACMD_DEF(trade),
		ACMD_DEF(send),
		ACMD_DEF2("rentitem", rentitem),
		ACMD_DEF2("rentitem2", rentitem2),
		...
		ACMD_DEF(allowks),
		ACMD_DEF(points), // Judas Points
		ACMD_DEF(cash),
		ACMD_DEF2("kpoints", cash),
		ACMD_DEF(agitstart2),
		ACMD_DEF(agitend2),
		ACMD_DEF(resetskill),
		ACMD_DEF(resetstat),
