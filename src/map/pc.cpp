 ...
 */
int pc_getpoints(struct map_session_data *sd, const char *message, int points)
{
	nullpo_retr(-1, sd);
	...
	for (int x = 0; x < jDB->points; x++) {
		if (strcmp(message, jDB->jpc[x]->the_variable) == 0) {
			foundIt = 1;
			theIndex = x;
			break;
		}
	...
	if (!strcmpi(message, jDB->jpc[theIndex]->the_variable)) {
		//if (points > MAX_ZENY - sd->AcctRokPoints)
		if (points > MAX_ZENY - pc_readaccountreg(sd, add_str(jDB->jpc[theIndex]->the_variable)))
		{
			ShowWarning("pc_getpoints: Cash point overflow (cash=%d, have cash=%d, account_id=%d, char_id=%d).\n", points, pc_readaccountreg(sd, add_str(jDB->jpc[theIndex]->the_variable)), sd->status.account_id, sd->status.char_id);
			points = MAX_ZENY - pc_readaccountreg(sd, add_str(jDB->jpc[theIndex]->the_variable));
		}

		pc_setaccountreg(sd, add_str(jDB->jpc[theIndex]->the_variable), pc_readaccountreg(sd, add_str(jDB->jpc[theIndex]->the_variable)) + points);


		return points;
	}
	else {
		clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], "Invalid syntax used", false, SELF);
	}
 ...
 */
int pc_paycash(struct map_session_data *sd, int price, int points, e_log_pick_type type, const char* str)
{
	int cash;
	nullpo_retr(-1,sd);
	...
	if (str != NULL) {
		...
		for (int x = 0; x < jDB->points; x++) {
			if (strcmp(str, jDB->jpc[x]->the_variable) == 0) {
		...
		if (pc_readaccountreg(sd, add_str(jDB->jpc[theIndex]->the_variable)) < cash || sd->kafraPoints < points) {
			ShowError("pc_paycash: Not enough points (cash=%d, kafra=%d) to cover the price (cash=%d, kafra=%d) (account_id=%d, char_id=%d).\n", pc_readaccountreg(sd, add_str(jDB->jpc[theIndex]->the_variable)), sd->kafraPoints, cash, points, sd->status.account_id, sd->status.char_id);
			return -1;
		}
		pc_setaccountreg(sd,add_str(str), pc_readaccountreg(sd, add_str(jDB->jpc[theIndex]->the_variable)) - cash);
	} else {
	...
	if( battle_config.cashshop_show_points && battle_config.cashshop_db != 2)
	{
...
static void pc_calcexp(struct map_session_data *sd, unsigned int *base_exp, unsigned int *job_exp, struct block_list *src)
{
	...
	if (src) {
		...
		if (sd->group_id == battle_config.partner_account) { // Modified Partner Account
			vip_bonus_base = battle_config.partner_base_exp_increase;
			vip_bonus_job = battle_config.partner_job_exp_increase;
		}
	}
...
int pc_dead(struct map_session_data *sd,struct block_list *src)
{
		...
		&& !mapdata->flag[MF_NOEXPPENALTY] && !mapdata_flag_gvg2(mapdata))
	{
		...
		}else if ( sd->group_id == battle_config.partner_account ) { // Modified Partner Account
			base_penalty = battle_config.partner_exp_penalty_base;
			job_penalty = battle_config.partner_exp_penalty_job;
		} else {
...
bool pc_candrop(struct map_session_data *sd, struct item *item)
{
	...
	if( !pc_disable_drop_items(sd) || sd->sc.cant.drop) //check if this GM level can drop items
		clif_displaymessage (sd->fd, "Your are not authorize to drop items.");
		return false;
	if( item->card[0] == CARD0_CREATE) {
