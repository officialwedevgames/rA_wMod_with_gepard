...
// Nasagnilac Mod start
		unsigned int roulette_reloaded; // Judas Reload Roulette
// Nasagnilac Mod end
	} state;
	struct {
...
#define pc_isidle(sd)         ( (sd)->chatID || (sd)->state.vending || (sd)->state.buyingstore || DIFF_TICK(last_tick, (sd)->idletime) >= battle_config.idle_no_share || (sd)->state.autotrade )
...
#define pc_can_trade_rental(sd) ( pc_has_permission((sd),PC_PERM_TRADE_RENTAL) ) // Judas Rental
