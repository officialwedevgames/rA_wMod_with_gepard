...
#else
		// Modify calculated base/job exp for VIP
		if (sd[i]->vip.enabled && pc_has_permission(sd[i], PC_PERM_VIP_PARTY_BONUS)) {
			base_exp = (unsigned int)cap_value(base_exp * (1 + (battle_config.vip_party_bonus / 100.)), 0, UINT_MAX);
			job_exp = (unsigned int)cap_value(job_exp * (1 + (battle_config.vip_party_bonus / 100.)), 0, UINT_MAX);
		}
