...
bool cashshop_buylist( struct map_session_data* sd, uint32 kafrapoints, int n, uint16* item_list ){
	...
	if (battle_config.cashshop_db == 1) { // 1 = acc_reg_num_db {
		if ((sd->cashPoints + kafrapoints) < totalcash) {
			clif_cashshop_result(sd, 0, CASHSHOP_RESULT_ERROR_SHORTTAGE_CASH);
			return false;
		}
	}
	else if (battle_config.cashshop_db == 2) { // 2 = points_account_db
		if ((pc_readaccountreg(sd, add_str("#ROKPOINTS")) + kafrapoints) < totalcash) {
			clif_cashshop_result(sd, 0, CASHSHOP_RESULT_ERROR_SHORTTAGE_CASH);
			return false;
		}
	}

	pc_paycash(sd, totalcash, kafrapoints, LOG_TYPE_CASH, "#ROKPOINTS");// [Ryuuzaki]
	...
	// Judas Tier
	if (SQL_ERROR == Sql_Query(mmysql_handle, "SELECT account_id FROM tier_db WHERE account_id = '%d'", sd->status.account_id)) {
		Sql_ShowDebug(mmysql_handle);
	}
